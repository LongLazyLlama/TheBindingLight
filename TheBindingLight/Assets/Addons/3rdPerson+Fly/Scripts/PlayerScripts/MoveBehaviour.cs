...
public class MoveBehaviour : GenericBehaviour
{
	...
	private int jumpLocalBool;                           // Animator variable related to jumping.
	...
	void Start()
	{
		...
		jumpLocalBool = Animator.StringToHash("LocalJump");
	...
	{
		// Get jump input.
		if (context.ReadValueAsButton() && !jump && behaviourManager.IsCurrentBehaviour(this.behaviourCode) && !behaviourManager.IsOverriding())
		{
			jump = true;
		}
	}
	...
	void JumpManagement()
	{
		...
		if (jump && !behaviourManager.GetAnim.GetBool(jumpBool) && behaviourManager.IsGrounded())
		{
			...
			LocalJump = behaviourManager.GetAnim.GetFloat(speedFloat) < 0.1;
			behaviourManager.GetAnim.SetBool(jumpLocalBool, LocalJump);
			if (behaviourManager.GetAnim.GetFloat(speedFloat) > 0.1 || true)
			{
		...
		else if (behaviourManager.GetAnim.GetBool(jumpBool))
		{
			...
			if (!LocalJump && !behaviourManager.IsGrounded() && !isColliding && behaviourManager.GetTempLockStatus())
			{
	...
	void MovementManagement(float horizontal, float vertical)
	{
		...
		{
			speed += behaviourManager.SprintAmount * sprintSpeed;
		}
